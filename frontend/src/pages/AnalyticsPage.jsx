...
export default function AnalyticsPage() {
    ...
    const transparencyColor = transparency?.transparencyIndex >= 70 ? 'text-emerald-500' : transparency?.transparencyIndex >= 40 ? 'text-amber-500' : 'text-red-500';
        ...
        <div className="page-container">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div className="glass-card-solid p-6 flex flex-col items-center justify-center animate-slide-up">
                    ...
                    <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Mess Transparency Index</h3>
                    <div className="relative w-28 h-28">
                        <svg className="w-28 h-28 -rotate-90" viewBox="0 0 100 100">
                            <circle cx="50" cy="50" r="45" fill="none" stroke="currentColor" strokeWidth="8" className="text-gray-100 dark:text-dark-700" />
                            <circle cx="50" cy="50" r="45" fill="none" stroke="currentColor" strokeWidth="8" strokeLinecap="round" className={transparencyColor} strokeDasharray={circumference} strokeDashoffset={offset} style={{ transition: 'stroke-dashoffset 1.5s ease-out' }} />
                        </svg>
                        <div className="absolute inset-0 flex items-center justify-center">
                            <span className={`text-2xl font-bold ${transparencyColor}`}>{transparency?.transparencyIndex || 0}%</span>
                        </div>
                    </div>
                ...
                <div className="glass-card-solid p-6 animate-slide-up" style={{ animationDelay: '100ms' }}>
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Predicted Next Month</h3>
                    </div>
                    <p className="text-3xl font-bold text-gray-900 dark:text-white">₹{(prediction?.predictedNextMonth || 0).toLocaleString()}</p>
                    <div className="mt-3 flex items-center gap-2">
                        <span className={`badge ${prediction?.confidence === 'medium' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'}`}>
                ...
                <div className="glass-card-solid p-6 animate-slide-up" style={{ animationDelay: '200ms' }}>
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Wastage Estimate</h3>
                    </div>
                    <p className="text-3xl font-bold text-gray-900 dark:text-white">{wastage?.wastagePercent || 0}%</p>
                    <div className="mt-3 w-full h-3 rounded-full bg-gray-100 dark:bg-dark-700 overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all duration-1000 ${(wastage?.wastagePercent || 0) > 20 ? 'bg-gradient-to-r from-red-400 to-red-500' : 'bg-gradient-to-r from-emerald-400 to-emerald-500'
                                }`}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-card-solid p-6 animate-slide-up" style={{ animationDelay: '300ms' }}>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Monthly Expense Trend</h3>
                ...
                <div className="glass-card-solid p-6 animate-slide-up" style={{ animationDelay: '400ms' }}>
                    ...
                    <ResponsiveContainer width="100%" height={280}>
                        <PieChart>
                            <Pie data={categoryBreakdown} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={4} dataKey="value">
                ...
                <div className="glass-card-solid p-6 lg:col-span-2 animate-slide-up" style={{ animationDelay: '500ms' }}>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Cost Per Plate Trend</h3>
                    <ResponsiveContainer width="100%" height={280}>
                        <BarChart data={costTrend}>
                            ...
                            <Tooltip
                                contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }}
                                formatter={(value, name) => [name === 'costPerPlate' ? `₹${value}` : value.toLocaleString(), name === 'costPerPlate' ? 'Cost/Plate' : 'Meals']}
                            />
                            <Legend />
                            <Bar dataKey="costPerPlate" fill="#8b5cf6" radius={[8, 8, 0, 0]} name="Cost Per Plate (₹)" />
