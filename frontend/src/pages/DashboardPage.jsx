...
import { HiOutlineUserGroup, HiOutlineCurrencyRupee, HiOutlineChartBar, HiOutlineStar, HiOutlineCalendar, HiOutlineTrendingUp } from 'react-icons/hi';
        ...
        <div className="page-container">
            ...
            <div className="mb-8 animate-fade-in">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
                    Welcome back, <span className="bg-gradient-to-r from-primary-500 to-primary-700 bg-clip-text text-transparent">{user?.name?.split(' ')[0]}</span> 👋
                </h1>
                <p className="text-gray-500 dark:text-gray-400 mt-1">Here's what's happening in your mess today</p>
            </div>
                ...
                {statCards.map((card, i) => (
                    <div key={i} className={`stat-card animate-slide-up`} style={{ animationDelay: `${i * 100}ms` }}>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className={`p-2 rounded-xl bg-gradient-to-br ${card.color} ${card.shadow} shadow-lg`}>
            ...
            {/* Today's Menu */}
            <div className="glass-card-solid p-6 mb-8 animate-slide-up" style={{ animationDelay: '400ms' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {['breakfast', 'lunch', 'dinner'].map((meal) => (
                        <div key={meal} className="p-4 rounded-xl bg-gray-50 dark:bg-dark-700/50 border border-gray-100 dark:border-dark-600">
                            <div className="flex items-center gap-2 mb-2">
                                <span className="text-xl">{mealEmoji[meal]}</span>
                                <h3 className="font-semibold text-gray-700 dark:text-gray-200 capitalize">{meal}</h3>
                            </div>
                            <p className="text-sm text-gray-500 dark:text-gray-400">{menu?.[meal] || 'Not set'}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-8">
                ...
                <div className="glass-card-solid p-6 animate-slide-up" style={{ animationDelay: '500ms' }}>
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Transparency Breakdown</h2>
                    </div>
                    <div className="space-y-4">
                        {[
                            { label: 'Expense Tracking', value: stats?.transparency?.factors?.expenseTracking || 0, color: 'bg-emerald-500' },
                            { label: 'Attendance Tracking', value: stats?.transparency?.factors?.attendanceTracking || 0, color: 'bg-blue-500' },
                            { label: 'Feedback Engagement', value: stats?.transparency?.factors?.feedbackEngagement || 0, color: 'bg-violet-500' },
                        ].map((factor) => (
