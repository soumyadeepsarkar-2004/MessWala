...
import { HiOutlineMail, HiOutlineKey, HiOutlineLockClosed, HiOutlineArrowLeft } from 'react-icons/hi';
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
            {[1, 2, 3].map((s) => (
                <div key={s} className="flex items-center gap-2">
                    ...
                    {s < 3 && (
                        <div className={`w-8 h-0.5 ${step > s ? 'bg-primary-500' : 'bg-gray-200 dark:bg-dark-700'}`} />
                    )}
                </div>
            ))}
        </div>
        ...
        <div className="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-primary-50 via-white to-orange-50 dark:from-dark-950 dark:via-dark-900 dark:to-dark-950">
            ...
            <div className="w-full max-w-md relative animate-fade-in">
                ...
                <div className="glass-card-solid p-8">
                    {step === 1 && (
                        <form onSubmit={handleSendOTP} className="space-y-5">
                            ...
                            <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
                                {loading ? <div className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> : 'Send OTP'}
                            </button>
                        </form>
                    )}
                    ...
                    {step === 2 && (
                        <form onSubmit={handleVerifyOTP} className="space-y-5">
                            ...
                            <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
                                {loading ? <div className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> : 'Verify OTP'}
                            </button>
                            <button
                                type="button"
                ...
                <div className="text-center mt-4">
                    <Link to="/login" className="inline-flex items-center gap-1 text-sm text-primary-500 hover:text-primary-600 dark:text-primary-400 font-medium transition-colors">
