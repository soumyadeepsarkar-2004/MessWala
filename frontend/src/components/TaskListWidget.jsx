...
import { HiOutlinePlus, HiOutlineTrash, HiOutlinePencil, HiOutlineFilter, HiOutlineSortDescending, HiOutlineExclamation, HiOutlineCalendar, HiOutlineX, HiOutlineFlag, HiOutlineClipboardList } from 'react-icons/hi';
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white dark:bg-dark-800 rounded-2xl shadow-2xl max-w-sm w-full p-6 animate-scale-in border border-gray-200 dark:border-dark-600">
                ...
                <div className="flex justify-end gap-3 mt-6">
                    ...
                    <button onClick={onConfirm} className="btn-danger text-sm py-2 px-4">Delete Permanently</button>
                </div>
            </div>
        </div>
    );
            ...
            <div className="relative bg-white dark:bg-dark-800 rounded-2xl shadow-2xl max-w-lg w-full p-6 animate-scale-in border border-gray-200 dark:border-dark-600">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        <button type="button" onClick={onClose} className="btn-secondary text-sm py-2 px-5">Cancel</button>
    ...
    if (loading) {
        return (
            <div className="space-y-4">
                {[...Array(3)].map((_, i) => (
                    <div key={i} className="p-5 border border-gray-100 dark:border-dark-700 rounded-xl animate-pulse">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        {[
                            ...
                            <button
                                ...
                                className={`px-2 py-1 rounded text-xs font-medium transition-all ${filter === f.key
                ...
                {tasks.length === 0 ? (
                    ...
                    tasks.map((task, i) => {
                        ...
                        return (
                            <div
                                key={task._id}
                                className={`relative p-4 rounded-xl border border-gray-100 dark:border-dark-700 bg-gray-50/50 dark:bg-dark-800/50 transition-all duration-700 ${isAnimating ? 'bg-emerald-50/80 dark:bg-emerald-900/10 scale-[0.98] opacity-80' : ''
                                    } ${task.completed && !isAnimating ? 'opacity-70' : ''}`}
                            >
                                <div className={`absolute top-0 bottom-0 left-0 w-1 rounded-l-xl bg-gradient-to-b ${priority.stripe}`} />
                                <div className="flex items-start gap-3 pl-2">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            <span className={`text-sm font-semibold text-gray-900 dark:text-white ${task.completed ? 'line-through text-gray-400 dark:text-gray-500' : ''}`}>
                                                {task.title}
                                            </span>
                                            <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold border ${priority.color}`}>
                                                {priority.label}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-3 mt-1.5 text-[11px]">
                                            {task.dueDate && (
                                                <span className={`flex items-center gap-1 ${dueDateColors[dueDateStatus] || 'text-gray-400'}`}>
                                                    <HiOutlineCalendar className="w-3 h-3" />
                                                    {dueDateStatus === 'overdue' && '⚠️ '}
                                                    {new Date(task.dueDate + 'T00:00').toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center flex-shrink-0 gap-1 opacity-0 group-hover:opacity-100 md:opacity-100 transition-opacity">
                                        <button onClick={() => { setEditingTask(task); setShowForm(true); }} className="p-1 text-gray-400 hover:text-blue-500 transition-colors">
                                            <HiOutlinePencil className="w-3.5 h-3.5" />
                                        </button>
                                        <button onClick={() => setDeleteTarget(task)} className="p-1 text-gray-400 hover:text-red-500 transition-colors">
