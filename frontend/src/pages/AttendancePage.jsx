                            ...
                            <div className="p-6">
                                ...
                                <div className="flex gap-3">
                                    <button
                                        ...
                                        className={`flex-1 py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-300 ${isPresent
                                ...
                                {isSkipped && (
                                    <p className="text-center text-sm text-red-400 mt-3 animate-fade-in">Skipped this meal</p>
                                )}
                            </div>
                        </div>
                    );
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-100 dark:border-dark-700">
                                ...
                                <th className="text-center py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">🌅 Breakfast</th>
                                <th className="text-center py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">☀️ Lunch</th>
                                <th className="text-center py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">🌙 Dinner</th>
                            </tr>
                        </thead>
                        <tbody>
                            {Object.entries(historyByDate)
                                ...
                                .slice(0, 14)
                                .map(([date, meals]) => (
                                    <tr key={date} className="border-b border-gray-50 dark:border-dark-800 hover:bg-gray-50 dark:hover:bg-dark-800/50 transition-colors">
                                        ...
                                        {['breakfast', 'lunch', 'dinner'].map((m) => (
                                            <td key={m} className="text-center py-3 px-4">
                                                {meals[m] === true && <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 text-xs">✓</span>}
                                                {meals[m] === false && <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-red-100 dark:bg-red-900/30 text-red-500 text-xs">✗</span>}
                                                {meals[m] === undefined && <span className="text-gray-300 dark:text-dark-600">—</span>}
                                            </td>
                                        ))}
                                    </tr>
                                ))}
