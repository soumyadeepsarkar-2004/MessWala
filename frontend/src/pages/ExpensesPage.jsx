...
export default function ExpensesPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setForm({ category: 'vegetables', amount: '', description: '', date: new Date().toISOString().split('T')[0] });
            setShowForm(false);
            fetchData();
        } catch (err) {
        ...
        <div className="page-container">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 animate-fade-in">
                ...
                <div className="flex gap-3 mt-4 sm:mt-0">
                    {canManage && (
                        <button onClick={() => setShowForm(!showForm)} className="btn-primary flex items-center gap-2" id="add-expense-btn">
                            <HiOutlinePlus className="w-4 h-4" />
                            Add Expense
                        </button>
                    )}
                    <button onClick={handleExport} className="btn-secondary flex items-center gap-2" id="export-csv-btn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="stat-card bg-gradient-to-br from-primary-500 to-primary-600 text-white border-0 animate-slide-up">
                    ...
                    <p className="text-primary-200 text-xs mt-1">{costPerPlate?.mealsServed || 0} meals served this month</p>
                </div>
                <div className="stat-card animate-slide-up" style={{ animationDelay: '100ms' }}>
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Total Monthly Expense</p>
                    <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">₹{(summary?.totalExpense || 0).toLocaleString()}</p>
                ...
                <div className="stat-card animate-slide-up" style={{ animationDelay: '200ms' }}>
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{summary?.breakdown?.length || 0}</p>
            ...
            {showForm && (
                <div className="glass-card-solid p-6 mb-8 animate-scale-in">
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <input type="number" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="Amount (₹)" className="input-field" required min="0" id="expense-amount" />
                        <input type="text" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Description" className="input-field" id="expense-description" />
                        <div className="flex gap-2">
                            <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="input-field flex-1" id="expense-date" />
                            <button type="submit" className="btn-primary whitespace-nowrap" id="expense-submit">Add</button>
                        </div>
                    </form>
                </div>
            )}
            ...
            {/* Category breakdown */}
            <div className="glass-card-solid p-6 mb-8 animate-slide-up" style={{ animationDelay: '300ms' }}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {summary?.breakdown?.map((b) => {
                        ...
                        const percent = summary.totalExpense > 0 ? ((b.total / summary.totalExpense) * 100).toFixed(1) : 0;
                        return (
            ...
            <div className="glass-card-solid p-6 animate-slide-up" style={{ animationDelay: '400ms' }}>
                ...
                <div className="space-y-3">
                    {expenses.slice(0, 20).map((exp) => {
                        ...
                        return (
                            <div key={exp._id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-dark-700/50 transition-colors group">
                                <div className="flex items-center gap-3">
                                    <span className={`px-2.5 py-1 rounded-lg text-xs font-medium ${info.color}`}>
                                        {info.label}
                                    </span>
                                    <div>
                                        <p className="text-sm font-medium text-gray-700 dark:text-gray-200">{exp.description || 'No description'}</p>
                                        <p className="text-xs text-gray-400">{new Date(exp.date + 'T00:00').toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="text-sm font-bold text-gray-900 dark:text-white">₹{exp.amount.toLocaleString()}</span>
                                    {canManage && (
                                        <button
